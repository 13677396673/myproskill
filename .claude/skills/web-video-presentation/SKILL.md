...
└── presentation/       # 脚手架产出的 Vite + React + TS 项目
    ...
    │   └── tts-providers/          # 每 provider 一个 .sh（内置 4 个）
    │       ├── README.md           # 三函数契约 + 5 段现成代码片段（11labs / edge-tts / say / azure / gcloud）
    │       ├── minimax.sh          # 默认 provider，用 mmx-cli
    │       ├── mimo.sh             # 小米 MiMo-V2.5-TTS（chat-completions 接口，base64 回包）
    │       ├── mimo-voiceclone.sh  # 小米 MiMo-V2.5-TTS 音色克隆（需参考音频样本）
...
| Phase 3 音频合成 | `references/AUDIO.md`（含 narrations.ts → segments.json → 任意 provider 流程，内置 minimax + openai + mimo） | `templates/scripts/tts-providers/README.md`（换 provider / 自带 TTS 时） |
...
要不要合成音频做"自动播放录屏"？
  ✓ 合成 → 扫所有章节的 narrations.ts 出 audio-segments.json，
           ...
           内置四个 provider：
             ...
             • mimo    (MIMO_API_KEY)      —— 小米 MiMo-V2.5-TTS，中文音色丰富，免费试用
             • mimo-voiceclone (MIMO_API_KEY) —— 音色克隆，需参考音频样本
...
# 或用内置 mimo (要 MIMO_API_KEY):
PRESENTATION_TTS=mimo npm run synthesize-audio -- --voice=冰糖
# 或用内置 mimo-voiceclone (音色克隆，要 MIMO_API_KEY + 参考音频):
PRESENTATION_TTS=mimo-voiceclone npm run synthesize-audio -- --voice=my-sample
...
| [`templates/scripts/tts-providers/README.md`](templates/scripts/tts-providers/README.md) | 换 / 加 TTS provider 时 | 三函数契约 + 内置 4 个 (minimax / openai / mimo / mimo-voiceclone) + 5 种现成代码片段（ElevenLabs / edge-tts / macOS say / Azure / Google） |
